...
All requests made to this API **require** your API [private secret](concepts.html#api-keys) in the header for authentication. Requests made to the public API, which are facilitated by methods in the [JavaScript client](frontend/javascript), will instead require your API [public key](concepts.html#api-keys).
...
### Request
`POST` requests made to the `/signin` endpoint unpack a [verification token](concepts.html#tokens), which must be generated by calling a `.signinWith*()` method on your frontend ([learn more](frontend/javascript.html#signin)) and included here in the request body, for example:
